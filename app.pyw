...
ENGINE_PY = APP_DIR / "run_copy_engine.py"
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._sizing_pct = 0
        # FVG baseline + regime + prob engine state
        self._regime = "---"; self._regime_vol = 0; self._regime_trend = 0
        self._fvg_baseline = 0; self._fvg_gap = 0; self._fvg_status = "IDLE"
        self._fvg_dir = ""; self._btc_dist = 0
        self._prob_fair = 0; self._prob_pct = 0; self._prob_vol = 0; self._prob_edge = 0
        self._exec_path = ""; self._dca_status = ""
    ...
    def _build(self):
        ...
        # FVG + Prob + Regime panel
        fpr = tk.LabelFrame(self, text=" probability engine + FVG baseline + regime ",
                            font=("Consolas", 8, "bold"), fg=PURPLE, bg=BG2,
                            highlightbackground=BORDER, highlightthickness=1)
        fpr.pack(fill="x", padx=14, pady=4)
        fpr.columnconfigure(0, weight=1)
        fpr.columnconfigure(1, weight=1)
        fpr.columnconfigure(2, weight=1)

        # Regime
        rf = tk.Frame(fpr, bg=BG2)
        rf.grid(row=0, column=0, sticky="nsew", padx=8, pady=6)
        tk.Label(rf, text="REGIME", font=("Consolas", 7), fg=DIM, bg=BG2).pack(anchor="w")
        self._regime_l = tk.Label(rf, text="---", font=("Consolas", 14, "bold"), fg=DIM, bg=BG2)
        self._regime_l.pack(anchor="w")
        self._regime_d = tk.Label(rf, text="", font=("Consolas", 7), fg=DIM, bg=BG2)
        self._regime_d.pack(anchor="w")
        self._exec_l = tk.Label(rf, text="", font=("Consolas", 8, "bold"), fg=DIM, bg=BG2)
        self._exec_l.pack(anchor="w", pady=(4, 0))

        # Prob Engine
        pef = tk.Frame(fpr, bg=BG2)
        pef.grid(row=0, column=1, sticky="nsew", padx=8, pady=6)
        tk.Label(pef, text="PROB ENGINE", font=("Consolas", 7), fg=DIM, bg=BG2).pack(anchor="w")
        self._prob_l = tk.Label(pef, text="--% | fair --c", font=("Consolas", 11, "bold"), fg=DIM, bg=BG2)
        self._prob_l.pack(anchor="w")
        self._prob_d = tk.Label(pef, text="", font=("Consolas", 7), fg=DIM, bg=BG2)
        self._prob_d.pack(anchor="w")
        self._prob_bar = AnimBar(pef, w=160, h=6)
        self._prob_bar.pack(anchor="w", pady=(2, 0))

        # FVG Baseline
        ff = tk.Frame(fpr, bg=BG2)
        ff.grid(row=0, column=2, sticky="nsew", padx=8, pady=6)
        tk.Label(ff, text="FVG BASELINE", font=("Consolas", 7), fg=DIM, bg=BG2).pack(anchor="w")
        self._fvg_l = tk.Label(ff, text="base --c | fvg --", font=("Consolas", 11, "bold"), fg=DIM, bg=BG2)
        self._fvg_l.pack(anchor="w")
        self._fvg_d = tk.Label(ff, text="", font=("Consolas", 7), fg=DIM, bg=BG2)
        self._fvg_d.pack(anchor="w")
        self._fvg_stat_l = tk.Label(ff, text="IDLE", font=("Consolas", 9, "bold"), fg=DIM, bg=BG2)
        self._fvg_stat_l.pack(anchor="w", pady=(2, 0))
    ...
    def _refresh(self):
        ...
        # Regime
        rc = {"TRENDING": GREEN, "MEAN_REVERTING": CYAN, "EXPLOSIVE": RED}.get(self._regime, DIM)
        self._regime_l.configure(text=self._regime, fg=rc)
        self._regime_d.configure(text=f"vol={self._regime_vol:.1f}% trend={self._regime_trend:.2f}")
        self._exec_l.configure(text=f"PATH: {self._exec_path}" if self._exec_path else "",
                              fg=GREEN if "CLEAN" in self._exec_path else CYAN if "MODERATE" in self._exec_path
                              else RED if "EXPLOSIVE" in self._exec_path else DIM)

        # Prob Engine
        pc2 = GREEN if self._prob_pct >= 65 or self._prob_pct <= 35 else ORANGE if self._prob_pct >= 55 or self._prob_pct <= 45 else DIM
        self._prob_l.configure(text=f"{self._prob_pct:.0f}% | fair {self._prob_fair:.0f}c", fg=pc2)
        self._prob_d.configure(text=f"edge={self._prob_edge:+.0f}c vol={self._prob_vol:.1f}%")
        self._prob_bar.set(self._prob_pct / 100 if self._prob_pct else 0.5, pc2)

        # FVG
        fc = GREEN if self._fvg_gap > 5 else RED if self._fvg_gap < -5 else DIM
        self._fvg_l.configure(text=f"base {self._fvg_baseline}c | fvg {self._fvg_gap:+.0f}c", fg=fc)
        self._fvg_d.configure(text=f"btc_dist={self._btc_dist:.3f}%" if self._btc_dist else "")
        sc = GREEN if self._fvg_status == "ENTERED" else YELLOW if self._fvg_status == "ARMED" else DIM
        self._fvg_stat_l.configure(text=self._fvg_status or "IDLE", fg=sc)
    ...
    def _parse(self, line):
        ...
        # ── New engine log parsers ──

        # Regime
        m = re.search(r'REGIME: (\w+) \| vol=([0-9.]+)% trend_score=([0-9.-]+)', line)
        if m:
            self._regime = m.group(1)
            self._regime_vol = float(m.group(2))
            self._regime_trend = float(m.group(3))

        # Prob engine
        m = re.search(r'PROB ENGINE: (YES|NO).*fair=(\d+)c.*mispricing=([0-9.+-]+)c.*prob=(\d+)%.*vol=([0-9.]+)%', line)
        if m:
            self._prob_fair = float(m.group(2))
            self._prob_edge = float(m.group(3))
            self._prob_pct = float(m.group(4))
            self._prob_vol = float(m.group(5))

        # FVG Baseline
        m = re.search(r'BASELINE: (\d+)c from (\d+) samples', line)
        if m:
            self._fvg_baseline = int(m.group(1))
            self._fvg_status = "BASELINE SET"

        # FVG Entry
        m = re.search(r'FVG ENTRY: (YES|NO).*fair=(\d+)c baseline=(\d+)c fvg=([0-9.+-]+)c.*btc_dist=([0-9.]+)%', line)
        if m:
            self._fvg_dir = m.group(1)
            self._fvg_gap = float(m.group(4))
            self._fvg_baseline = int(m.group(3))
            self._btc_dist = float(m.group(5))
            self._fvg_status = "ENTERED"
            self._log_add(f"FVG {m.group(1)} fair={m.group(2)}c base={m.group(3)}c gap={m.group(4)}c", "entry")

        # FVG Armed (waiting for tick confirm)
        m = re.search(r'FVG ARMED: (YES|NO).*vel=\$([0-9.+-]+)/s.*fvg=([0-9.+-]+)c', line)
        if m:
            self._fvg_dir = m.group(1)
            self._fvg_status = "ARMED"
            self._fvg_gap = float(m.group(3))

        # Sizing path
        m = re.search(r'SIZING: (\w+) path \| (\d+) contracts', line)
        if m:
            self._exec_path = m.group(1)
            self._log_add(f"SIZE {m.group(1)} {m.group(2)}x", "info")

        # TA_FORCED fill (current engine format)
        m = re.search(r'TA_FORCED FILL: (YES|NO).*?(\d+)x @ (\d+)c', line)
        if m:
            self._pos = f"{m.group(1)} {m.group(2)}x @ {m.group(3)}c"
            self._entry_px = int(m.group(3))
            self._mid_chart.set_entry(self._entry_px)
            self._log_add(f"FILL {m.group(1)} {m.group(2)}x @ {m.group(3)}c", "entry")

        # PROB TP placed
        m = re.search(r'PROB TP: (\d+)x @ (\d+)c.*fair=(\d+)c', line)
        if m:
            tp = int(m.group(2))
            self._mid_chart.set_tps([tp])
            self._log_add(f"TP set {m.group(1)}x @ {m.group(2)}c (fair={m.group(3)}c)", "info")

        # Dynamic TP moved
        m = re.search(r'DYNAMIC TP: moved (\d+)x (\d+)c.*?(\d+)c', line)
        if m:
            self._mid_chart.set_tps([int(m.group(3))])
            self._log_add(f"TP moved {m.group(2)}c -> {m.group(3)}c", "info")

        # DCA
        m = re.search(r'DCA TIER (\d+): \+(\d+)x @ (\d+)c.*avg.*?(\d+)c.*total (\d+)x', line)
        if m:
            self._dca_status = f"T{m.group(1)} +{m.group(2)}x"
            self._log_add(f"DCA T{m.group(1)} +{m.group(2)}x@{m.group(3)}c avg={m.group(4)}c total={m.group(5)}x", "entry")

        # DCA skips
        if "DCA SKIP" in line or "DCA WAIT" in line:
            reason = line.split("DCA ")[1][:50] if "DCA " in line else ""
            self._dca_status = "SKIP"

        # Profit trail
        m = re.search(r'PROFIT TRAIL LOCK: (\d+)x @ (\d+)c.*\$([0-9.+-]+)', line)
        if m:
            self._log_add(f"TRAIL LOCK {m.group(1)}x@{m.group(2)}c ${m.group(3)}", "win")

        # Orphan catch
        m = re.search(r'ORPHAN SOLD: (\d+)x @ (\d+)c', line)
        if m:
            self._log_add(f"ORPHAN SOLD {m.group(1)}x@{m.group(2)}c", "exit")

        # Aggregated sell
        m = re.search(r'AGG SELL: re-placed (\d+)x @ (\d+)c', line)
        if m:
            self._log_add(f"AGG SELL {m.group(1)}x@{m.group(2)}c", "info")

        # Position closed with re-entry
        m = re.search(r'position closed.*window OPEN for re-entry.*side=(\w+)', line)
        if m:
            self._pos = ""
            self._mid_chart.set_entry(0)
            self._mid_chart.set_tps([])
            self._fvg_status = "IDLE"
            self._log_add(f"CLOSED - window open for {m.group(1)} re-entry", "info")
